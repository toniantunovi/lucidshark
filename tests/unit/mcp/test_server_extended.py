...
class TestMCPServerCallToolHandler:
    ...
    async def test_dispatch_scan(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.scan = AsyncMock(return_value={"total_issues": 0, "instructions": []})
        mock_rc, ctx_token = _set_request_context()
        try:
            handler = server.server.request_handlers[CallToolRequest]
            req = CallToolRequest(
                method="tools/call",
                params={"name": "scan", "arguments": {"domains": ["linting"]}},
            )
            result = await handler(req)
    ...
    async def test_dispatch_check_file(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.check_file = AsyncMock(return_value={"total_issues": 0})
        mock_rc, ctx_token = _set_request_context()
        try:
            handler = server.server.request_handlers[CallToolRequest]
            req = CallToolRequest(
                method="tools/call",
                params={"name": "check_file", "arguments": {"file_path": "test.py"}},
            )
            result = await handler(req)
            parsed = json.loads(result.root.content[0].text)
    ...
    async def test_dispatch_get_fix_instructions(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.get_fix_instructions = AsyncMock(
        ...
        try:
            ...
            req = CallToolRequest(
                method="tools/call",
                params={"name": "get_fix_instructions", "arguments": {"issue_id": "issue-1"}},
            )
            result = await handler(req)
            parsed = json.loads(result.root.content[0].text)
    ...
    async def test_dispatch_apply_fix(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.apply_fix = AsyncMock(
        ...
        try:
            ...
            req = CallToolRequest(
                method="tools/call",
                params={"name": "apply_fix", "arguments": {"issue_id": "issue-1"}},
            )
            result = await handler(req)
            parsed = json.loads(result.root.content[0].text)
    ...
    async def test_dispatch_get_status(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.get_status = AsyncMock(
    ...
    async def test_dispatch_get_help(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.get_help = AsyncMock(
    ...
    async def test_dispatch_autoconfigure(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.autoconfigure = AsyncMock(
        ...
        try:
            ...
            req = CallToolRequest(
                method="tools/call",
                params={"name": "autoconfigure", "arguments": {}},
            )
            result = await handler(req)
            parsed = json.loads(result.root.content[0].text)
    ...
    async def test_dispatch_validate_config(self, tmp_path: Path) -> None:
        ...
        server = _make_server(tmp_path)
        server.executor.validate_config = AsyncMock(
        ...
        try:
            ...
            req = CallToolRequest(
                method="tools/call",
                params={"name": "validate_config", "arguments": {"config_path": "lucidshark.yml"}},
            )
            result = await handler(req)
            parsed = json.loads(result.root.content[0].text)
    ...
    async def test_dispatch_exception_returns_error(
        ...
        server = _make_server(tmp_path)
        server.executor.scan = AsyncMock(side_effect=RuntimeError("Boom"))
        mock_rc, ctx_token = _set_request_context()
        try:
            handler = server.server.request_handlers[CallToolRequest]
            req = CallToolRequest(
    ...
    async def test_progress_notification_with_token(
        ...
        server.executor.scan = AsyncMock(side_effect=mock_scan)
